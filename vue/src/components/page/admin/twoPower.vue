<template>
  <div>
    <x-header :left-options="{showBack: true}">站长管理后台</x-header>
...
<script>
  ...
  export default {
    ...
    created () {
      this.setTitle('站长管理后台');
      this.$store.dispatch('update', {
        'model': 'common',
        'key': 'slideRoute',
        'value': '/tab4'
      });
      if (true === this.$store.state.power.getDone &&  2 >= this.$store.state.power.adminID &&  1 === this.$store.state.power.loginAdmin){
        this.power.is  =  false;
      }
    },
    methods: {
